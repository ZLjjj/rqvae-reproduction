...
$outputFile = "完整5层SID分析_任务3_L1_Sinkhorn.md"
...
$totalMultiSeason = $l1l2l3l4l5Same + $l1l2l3l4Same + $l1l2l3Same + $l1l2Same + $l1Same + $allDifferent
...
| Prefix层级 | 相同数量 | 比例 | 说明 |
|---|---:|---:|---|
| **L1相同** | $l1Same | $([math]::Round($l1Same/$totalMultiSeason*100,2))% | 仅第1层token相同 |
| **L1L2相同** | $l1l2Same | $([math]::Round($l1l2Same/$totalMultiSeason*100,2))% | 前2层token相同 |
| **L1L2L3相同** | $l1l2l3Same | $([math]::Round($l1l2l3Same/$totalMultiSeason*100,2))% | 前3层token相同 |
| **L1L2L3L4相同** | $l1l2l3l4Same | $([math]::Round($l1l2l3l4Same/$totalMultiSeason*100,2))% | 前4层token相同 |
| **L1L2L3L4L5完全相同** | $l1l2l3l4l5Same | $([math]::Round($l1l2l3l4l5Same/$totalMultiSeason*100,2))% | 5层完全相同！ |
| **完全不同** | $allDifferent | $([math]::Round($allDifferent/$totalMultiSeason*100,2))% | L1就不同 |
...
# 案例2: L1L2L3L4相同
$report += "`n### 案例2: L1L2L3L4相同 (前10个)`n`n"
$count = 0
...
# 案例3: L1L2L3相同
$report += "`n### 案例3: L1L2L3相同 (前10个)`n`n"
$count = 0
...
1. **5层完全相同率**: $([math]::Round($l1l2l3l4l5Same/$totalMultiSeason*100,2))% ($l1l2l3l4l5Same/$totalMultiSeason)
   - 这些资源的不同季被分配了**完全相同的语义标识**
   - 意味着模型无法区分这些资源的不同季
   - 可能原因：元数据相似度极高、内容高度重复

2. **L1L2L3L4相同率**: $([math]::Round($l1l2l3l4Same/$totalMultiSeason*100,2))% ($l1l2l3l4Same/$totalMultiSeason)
   - 仅靠L5（最后一层）区分不同季
   - L5承担了季度区分的主要责任

3. **L1L2L3相同率**: $([math]::Round($l1l2l3Same/$totalMultiSeason*100,2))% ($l1l2l3Same/$totalMultiSeason)
   - L4和L5共同区分不同季

4. **L1L2相同率**: $([math]::Round($l1l2Same/$totalMultiSeason*100,2))% ($l1l2Same/$totalMultiSeason)
   - 后3层(L3-L5)用于区分季度

5. **L1相同率**: $([math]::Round($l1Same/$totalMultiSeason*100,2))% ($l1Same/$totalMultiSeason)
   - 系列归属在顶层有一定体现

6. **完全不同率**: $([math]::Round($allDifferent/$totalMultiSeason*100,2))% ($allDifferent/$totalMultiSeason)
   - 大部分系列资源被完全分散
...
**缺点**：
- 5层完全相同的资源无法通过SID区分
- 大部分系列资源依然分散（$([math]::Round($allDifferent/$totalMultiSeason*100,2))%完全不同）
