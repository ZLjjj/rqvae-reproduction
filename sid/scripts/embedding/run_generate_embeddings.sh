...
# Configurable defaults (override via env vars before running)
...
GEN_SCRIPT="${SCRIPT_DIR}/generate_embeddings.py"
...
# Merge shards
# Note: generate_embeddings.py requires --input even in --merge mode (argparse required=True).
python "${GEN_SCRIPT}" \
  --input "${INPUT}" \
  --output_dir "${OUTPUT_DIR}" \
  --domain "${DOMAIN}" \
