...
def parse_args():
    p = argparse.ArgumentParser(description="Train RQOPQVAE (3+2) in sid")
    ...
    p.add_argument("--ema_codebook", action="store_true", help="Enable post-step EMA smoothing for the first 3 RQ codebooks (OPQ layer unchanged)")
    p.add_argument("--ema_decay", type=float, default=0.99)
    p.add_argument("--ema_eps", type=float, default=1e-5)

    p.add_argument("--num_emb_list", type=int, nargs="+", default=[1024, 1024, 1024, 1024])
...
def main():
    ...
    if args.ema_codebook:
        ...
        rqopq = model.rqopq
        for i, layer in enumerate(rqopq.layers):
            if i >= 3:
                break
            vq = getattr(layer, "quantizer", None)
            if vq is None:
                raise ValueError(f"Unexpected layer type at i={i}: missing .quantizer")
